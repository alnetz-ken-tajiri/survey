...
import { Loader2, Search, ArrowLeft, FileText, CheckSquare } from "lucide-react"
...
export default function SurveyResponses({ params }: { params: { id: string } }) {
    ...
    const renderQuestionResponse = (question: Question, responses: Response[]) => {
        const response = responses.find((r) => {
            return r.responseDetails.some((detail) => {
                if (question.type === "FILE") {
                    return detail.textValue && detail.textValue.startsWith("/uploads/")
                } else if (question.type === "CHECKBOX") {
                    return detail.optionLabel && detail.optionValue
                } else if (question.type === "TEXT") {
                    return detail.textValue !== null && !detail.textValue.startsWith("/uploads/")
                }
                return false
            })
        })
        ...
        console.log(`Matching response for ${question.name}:`, response)
